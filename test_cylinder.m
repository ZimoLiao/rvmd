...
[phi, c, omega, energy, omega_iter, diff_iter] = ...
    rvmd(q_fluc, K, alpha, tol, N, init, initFreqMax);

save('data/test_cylinder_mode.mat','phi','c','omega','energy','omega_iter','diff_iter');
% load('data/test_cylinder_mode.mat','phi','c','omega','energy','omega_iter','diff_iter');

%% post-processing
phi_velocity = reshape(phi,[V,I,J,K]);
[xx,yy] = meshgrid(x,y);
...
for t = 1:T
    shift_mode_vel_rec(:,:,:,t) = squeeze(phi_velocity(:,:,:,1))*c(t,1);
    shift_mode_vor_rec(:,:,t) = squeeze(phi_vorticity(:,:,1))*c(t,1);
end
...
for t = 1:T
    inter_mode_vel_rec(:,:,:,t) = squeeze(phi_velocity(:,:,:,2))*c(t,2)+ ...
        squeeze(phi_velocity(:,:,:,3))*c(t,3)+ ...
        squeeze(phi_velocity(:,:,:,1))*c(t,1);
    inter_mode_vor_rec(:,:,t) = squeeze(phi_vorticity(:,:,2))*c(t,2)+ ...
        squeeze(phi_vorticity(:,:,3))*c(t,3)+ ...
        squeeze(phi_vorticity(:,:,1))*c(t,1);
end
...
for t = 1:T
    post_mode_vel_rec(:,:,:,t) = squeeze(phi_velocity(:,:,:,4))*c(t,4)+ ...
        squeeze(phi_velocity(:,:,:,5))*c(t,5)+ ...
        squeeze(phi_velocity(:,:,:,1))*c(t,1);
    post_mode_vor_rec(:,:,t) = squeeze(phi_vorticity(:,:,4))*c(t,4)+ ...
        squeeze(phi_vorticity(:,:,5))*c(t,5)+ ...
        squeeze(phi_vorticity(:,:,1))*c(t,1);
end
